...
insert into answers(text,subject_id,user_id)
    value ("Если это может кому-то помочь, вот перевод принятого ответа Crembo на pura Java.",1,1),
("Метод контроллера",2,2),
("Основной класс (только значащие биты):",3,3),
("Ниже будет работать метод перезапуска.",4,4)